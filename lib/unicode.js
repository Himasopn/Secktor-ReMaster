   ...
   encode: function (text) {
    ...
    for (i in text) {
     ...
     if (this.options.top) {
      ...
      for (
       var count = 0,
        len = this.options.maxHeight - Math.random() * ((this.options.randomization / 100) * this.options.maxHeight);
       count < len;
       count++
      ) {
   ...
   encode: function (text) {
    ...
    for (i in text) {
     if (text[i].charCodeAt(0) >= 33) {
      ...
      if (!first) {
       ch =
        String.fromCharCode(8239) + String.fromCharCode(160) + String.fromCharCode(160) + String.fromCharCode(8239) + ch
      }
     } else {
      ch = text[i]
     }
